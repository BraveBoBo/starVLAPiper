...
from mpc_piper_single import MPC_piper
from visualize_actions import ActionVisualizer, MultiCameraVisualizer, visualize_openvla_prediction
...
def xyz_rpy_to_noap(pose_6d):
    """
    将6维位姿(x,y,z,roll,pitch,yaw)转换为9维NOAP位姿
    
    输入: pose_6d [x, y, z, roll, pitch, yaw] (6维)
    输出: pose_9d [x, y, z, nx, ny, nz, ox, oy, oz] (9维)
    
    其中:
    - [x, y, z]: 位置
    - [nx, ny, nz]: 旋转矩阵第一列(normal vector)
    - [ox, oy, oz]: 旋转矩阵第二列(orientation vector)
    - 第三列(approach vector)可通过前两列叉积得到
    """
    x, y, z = pose_6d[0], pose_6d[1], pose_6d[2]
    roll, pitch, yaw = pose_6d[3], pose_6d[4], pose_6d[5]
    
    # 计算旋转矩阵
    R = rpy_to_rotation_matrix(roll, pitch, yaw)
    
    # 提取n和o向量(旋转矩阵的前两列)
    n_vec = R[:, 0]  # normal vector
    o_vec = R[:, 1]  # orientation vector
    
    # 组合成9维NOAP格式
    noap_pose = np.array([
        x, y, z,           # 位置
        n_vec[0], n_vec[1], n_vec[2],  # nx, ny, nz
        o_vec[0], o_vec[1], o_vec[2]   # ox, oy, oz
    ])
...
def conduct_action(mpc,action,action_last):
    action_joint = action[:6]
    action_grip = action[6]
    joint = [0,0,0,0,0,0]
    state = xyz_rpy_to_noap(action_joint)
    action_joint_last = action_last[:6]
    state_last = xyz_rpy_to_noap(action_joint_last)
    x = np.hstack([state, joint]).reshape(-1,1)
    x_init = np.hstack([state_last, joint]).reshape(-1,1)
    mpc.ur_move(x_init,x,action_grip)
...
if __name__ == "__main__":
    ...
    init_flag = False
    action_last = right1
    pub = rospy.Publisher('/master/joint_right', JointState, queue_size=10)
    mpc = MPC_piper(pub)
    
    # 初始化可视化器
    action_visualizer = ActionVisualizer(max_history=200)
    camera_visualizer = MultiCameraVisualizer(args.camera_names)
    ...
    while True and not rospy.is_shutdown():
        ...
        if action is not None:
            # 转换图像为base64格式用于可视化
            img_for_viz = result[3]  # 使用d455相机图像
            ...
            if isinstance(action, (list, np.ndarray)):
                ...
                # 如果是字典，提取动作数据
                action_array = action.get('action', action) if isinstance(action, dict) else action
                action_array = np.array(action_array) if not isinstance(action_array, np.ndarray) else action_array
            
            # 添加到可视化器
            action_visualizer.add_action(action_array)
            
            # 保存数据用于后续分析
            visualization_data['actions'].append(action_array.copy())
            visualization_data['images'].append(img_for_viz.copy())
            visualization_data['timestamps'].append(time.time())
            
            # 每10步进行一次可视化更新
            if len(action_visualizer.action_history) % 10 == 0:
                print(f"更新可视化... 当前步数: {len(action_visualizer.action_history)}")
                
                # 创建多相机图像字典
                image_dict = {
                    args.camera_names[0]: result[0],  # img_front
                    args.camera_names[1]: result[1],  # img_left  
                    args.camera_names[2]: result[2],  # img_right
                    args.camera_names[3]: result[3],  # img_d455
                }
                
                # 显示多相机图像（每20步显示一次）
                if len(action_visualizer.action_history) % 20 == 0:
                    camera_visualizer.display_multi_camera(image_dict, 
                        save_path=f'multi_camera_step_{len(action_visualizer.action_history)}.png')
                
                # 实时显示动作轨迹
                try:
                    action_visualizer.plot_real_time(action_array, img_for_viz)
                except Exception as e:
                    print(f"可视化更新错误: {e}")
        
        # 执行动作
        # conduct_action(mpc, action, action_last)
        action_last = action
        rate.sleep()
        
        # 每100步保存一次完整的轨迹图
        if len(action_visualizer.action_history) % 100 == 0:
            try:
                action_visualizer.plot_action_trajectory(
                    save_path=f'action_trajectory_step_{len(action_visualizer.action_history)}.png')
                print(f"保存轨迹图: action_trajectory_step_{len(action_visualizer.action_history)}.png")
            except Exception as e:
                print(f"保存轨迹图错误: {e}")
    ...
    try:
        # 保存最终轨迹
        action_visualizer.plot_action_trajectory(save_path='final_action_trajectory.png')
